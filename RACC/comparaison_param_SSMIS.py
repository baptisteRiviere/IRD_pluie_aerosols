...
if __name__ == "__main__":
    ...
    dictionnary = {}
    for row in data:
        capteur,incidence,freq,true_rain,TB = row["capteur"],row["incidence"],row["freq"],row["true_rain"],row["TB"]
        if capteur == "F17_SSMIS":  
            key = capteur+incidence
            if key in dictionnary.keys():
                rain = dictionnary[key]
            else :
                rain = [0,0,0]
            rain[0] = true_rain
            if freq == "91V":
                ...
                rain[2] = TB
            dictionnary[key] = rain

    TB91V,TB91H = [],[]
    diff_TB,encore_true_rain = [],[]
    for key in dictionnary.keys():
        if (dictionnary[key][1] != 0) and (dictionnary[key][2] != 0):
            #if dictionnary[key][0] < 1:
            calcul = 1.818*dictionnary[key][1] - 0.818*dictionnary[key][2]
            #calcul = dictionnary[key][1] - dictionnary[key][2]
            diff_TB.append(calcul)
            encore_true_rain.append(dictionnary[key][0])
            TB91V.append(dictionnary[key][1])
            TB91H.append(dictionnary[key][2])
    

    A=np.array([TB91V,TB91H,[1 for i in range(len(TB91V))]]).T
    B=np.array(encore_true_rain)
    ...
    print(X)
    #import scipy
    #f = scipy.interpolate.interp1d(A[:,0]-A[:,1], B, kind='linear')
    #print (f)


    corr, _ = spearmanr(diff_TB, encore_true_rain)
    corrV, _ = spearmanr(TB91V, encore_true_rain)
    corrH, _ = spearmanr(TB91H, encore_true_rain)

    print(corr,corrV,corrH)

    plt.scatter(encore_true_rain,diff_TB)
    plt.scatter(encore_true_rain,TB91V)
    plt.scatter(encore_true_rain,TB91H)
    plt.show()

    #print(np.mean(encore_true_rain))
    #print(np.std(encore_true_rain))
    #print(np.max(encore_true_rain))
    #print(np.min(encore_true_rain))
