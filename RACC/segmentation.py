...
def generate_X(path_list):
    """
    Generate X object fitted to the algorithm from the paths to images

    Args
        path_list (list): list of paths containing images
    ...
    for path in path_list:
        img = np.asarray(Image.open(path)) # ouverture des fichiers image
        plt.imshow(img)
        plt.show()
        X.append(img)
    X = np.array(X)
...
if __name__ == '__main__':
    # load images
    images_paths = glob.glob(r"../data/test_seg/*.tiff")
    X, (x,y) = generate_X(images_paths)
